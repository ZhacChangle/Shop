  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="detail">
        ...
        <ul
          class="list clearFix"
          v-for="(good, index) in tradeGoodsInfo.detailArrayList"
          :key="good.skuId"
        >
          <li>
            <img
    ...
    <div class="money clearFix">
      <ul>
        <li>
          <b
            ><i>{{ tradeGoodsInfo.totalNum }}</i
            >件商品，总商品金额</b
          >
          <span>¥{{ tradeGoodsInfo.totalAmount }}.00</span>
        </li>
        <li>
          <b>返现：</b>
          <span>0.00</span>
    ...
    <div class="trade">
      <div class="price">
        应付金额:　<span>¥{{ tradeGoodsInfo.totalAmount }}.00</span>
      </div>
...
export default {
  ...
  computed: {
    ...mapState({
      addressInfo: (state) => state.trade.tradeAddress,
      tradeGoodsInfo: (state) => state.trade.tradeInfo,
      userDefaultAddress() {
  ...
  methods: {
    ...
    async submitOrder() {
      let traderNo = this.tradeGoodsInfo.tradeNo;
      let data = {
      ...
      let result = await this.$API.reqOrderPay(traderNo, data);
      if (result.code == 200) {
        this.$router.push("/pay?traderNo=" + result.data);
      }
    },
  },
};
