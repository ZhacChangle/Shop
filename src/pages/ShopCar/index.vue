<template>
  <div class="cart">
        ...
        >
          <li class="cart-list-con1">
            <input
              ...
              @change="CheckCartById(cartInfo, $event)"
            />
          </li>
    ...
    <div class="cart-tool">
      ...
      <div class="money-box">
        <div class="chosed">
          已选择 <span>{{ this.cartInfoList.length }}</span
          >件商品
        </div>
        <div class="sumprice">
          <em>总价（不含运费） ：</em>
...
import throttle from "../../../node_modules/loadsh/throttle";
export default {
  name: "ShopCart",
  mounted() {
    this.getData();
  ...
  methods: {
    ...
    handler: throttle(async function (type, disNum, item) {
      switch (type) {
        case "mins":
          item.skuNum == 1 ? (disNum = 0) : (disNum = -1);
          break;
        case "change":
          if (isNaN(disNum) || disNum < 0) disNum = 0;
          else {
    ...
    async CheckCartById(cartInfo, event) {
      // console.log(cartInfo, event);
  ...
  computed: {
    ...
    totalPrice() {
      ...
      this.cartInfoList.forEach((element) => {
        if (element.isChecked == 1) sum += element.skuPrice * element.skuNum;
      });
      return sum;
    },
  },
